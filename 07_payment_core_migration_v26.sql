-- ============================================================================
-- LYANN — MIGRATION 07 V2.6 : PAYMENT CORE (LEGACY ADAPTATION & ESCROW SCHEMA)
-- ============================================================================
-- IMPORTANT : Ce script adapte la table legacy `public.payments` (actuellement vide)
-- et aligne `public.milestones` vers la machine d'état V2.6 finale.
...
-- 1. ALIGNEMENT DES CONTRAINTES DE PUBLIC.MILESTONES (STATUTS METIER V2.6)
...
ALTER TABLE public.milestones 
  ...
  CHECK (status IN ('PENDING', 'FUNDED', 'COMPLETED', 'RELEASED', 'DISPUTED', 'CANCELLED'));
...
-- 3. RESTRUCTURATION DES COLONNES DE PUBLIC.PAYMENTS
...
-- Suppression des anciennes colonnes legacy inutilisées (amount float & released_at)
ALTER TABLE public.payments DROP COLUMN IF EXISTS amount;

-- Ajout idempotent des colonnes financières structurées (Centimes entiers bigint & séparation statuts)
ALTER TABLE public.payments
...
-- Ré-association explicite des 5 clés étrangères indispensables avec ON DELETE RESTRICT
ALTER TABLE public.payments
...
-- 4. CONTRAINTES DE SÉCURITÉ ET COMPTABILITÉ FINANCIÈRE V2.6 (IDEMPOTENTES)
ALTER TABLE public.payments
  ...
  ADD CONSTRAINT chk_payments_transfer_status 
    CHECK (transfer_status IN ('NOT_STARTED', 'PENDING_VALIDATION', 'TRANSFER_PROCESSING', 'TRANSFERRED', 'TRANSFER_FAILED', 'REVERSED_PROCESSING', 'REVERSED')),
  ADD CONSTRAINT uq_payments_stripe_intent 
    UNIQUE (stripe_payment_intent_id);

-- 5. GESTION DES PAYMENT ATTEMPTS & UNICITÉ PAR JALON / PARTIE (INDEX UNIQUE PARTIEL V2.6)
...
-- 6. TRIGGER D'HORODATAGE UPDATED_AT (IDEMPOTENT)
...
-- 7. SÉCURITÉ RLS ET PERMISSIONS MATRICIELLES SUR PUBLIC.PAYMENTS
...
-- 8. TABLE D'IDEMPOTENCE DES WEBHOOKS STRIPE (IDEMPOTENTE)
