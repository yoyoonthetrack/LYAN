...
(function() {
    ...
    window.LYANN_PAYMENTS = {
        ...
        PAYMENT_MODE: (function() {
            try {
                if (window.NODE_ENV === 'production') return 'stripe_production';
                return window.LYANN_PAYMENT_MODE || (location.hostname === 'localhost' || location.hostname === '127.0.0.1' ? 'mock_dev' : 'stripe_production');
            } catch(e) {
        ...
        approveQuotationAndPay: async function(txId, hasProtection, paymentDetails) {
            ...
            if (this.PAYMENT_MODE === 'stripe_production') {
                try {
                    const res = await fetch('/v1/payments/create-intent', {
                        ...
                        body: JSON.stringify({
                            missionId: tx.id,
                            quoteId: tx.quoteDetails ? tx.quoteDetails.id : '',
                            amount: tx.amount,
                            currency: 'eur',
                            idempotencyKey: `pay_${tx.id}_${Date.now()}`
                        })
                    });
                    const intentData = await res.json();
                    if (intentData && intentData.clientSecret) {
                        tx.stripePaymentIntentId = intentData.paymentIntentId;
                        tx.status = "REQUIRES_PAYMENT";
                    }
                } catch(e) {
                    console.warn("Stripe Server PaymentIntent call failed, fallback to local:", e);
                    tx.status = "en_cours";
                }
            } else {
                tx.status = "en_cours";
            ...
            // Placer l'intégralité du solde des jalons dans le solde sécurisé
            const wallets = this.getWallets();
            ...
            // Notification de paiement sécurisé
            if (window.LYANN_NOTIFICATIONS) {
                window.LYANN_NOTIFICATIONS.sendSMS(
                    '+590690001122',
                    tx.providerName,
                    `Félicitations! Le client ${tx.customerName} a accepté votre devis. Les fonds (${tx.amount} €) sont sous protection sécurisée LYANN. Vous pouvez démarrer la prestation.`
                );
            }
