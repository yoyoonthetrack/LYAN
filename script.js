                ...
                try {
                    ...
                    if (isLyann) {
                        if (isOwnLyann) {
                            ...
                            ctaButtonHTML = `<button class="flash-action-btn btn-help-lyann" data-request-id="${targetId}" data-requester-id="${authorId}" data-requester-name="${authorDisplayName.replace(/"/g, '&quot;')}" data-requester-avatar="${post.author_avatar || post.authorAvatar || ''}" data-title="${(post.title || post.content || '').replace(/"/g, '&quot;')}" style="width: 100%; background: var(--primary, #4A7C59); color: #FFF; font-weight: 800; border-radius: 22px; min-height: 44px; padding: 0 16px; border: none; font-size: 0.85rem; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; gap: 6px; box-sizing: border-box;"><i class="ph ph-hand-heart"></i> <span>Je peux aider</span></button>`;
                        }
                    }

                    return `
                        <div class="flash-card ${isLyann ? 'lyann-card' : ''}" id="${post.id || ''}" style="${isLyann ? 'background: #FAF7F2; border-radius: 20px; padding: 12px 14px; margin-bottom: 10px; border: 1px solid rgba(74, 124, 89, 0.18); box-shadow: 0 2px 8px rgba(0,0,0,0.02); width: 100%; box-sizing: border-box; overflow: hidden;' : 'background: #FFFFFF; border-radius: 20px; padding: 12px 14px; margin-bottom: 10px; border: 1px solid #E2E8F0; box-shadow: 0 2px 8px rgba(0,0,0,0.02); width: 100%; box-sizing: border-box; overflow: hidden;'}">
                            <!-- HEADER ROW: AVATAR (44px) + IDENTITY/LOCATION/DATE (LEFT) | BADGE (RIGHT, TRUNCATED) -->
                            <div class="flash-card-header" style="display: flex; justify-content: space-between; align-items: flex-start; gap: 8px; margin-bottom: 8px; width: 100%; box-sizing: border-box;">
                                ...
                                <div class="flash-meta-badges" style="flex-shrink: 1; max-width: 45%; text-align: right; min-width: 0;">
                                    <span class="flash-badge-default" style="${isLyann ? 'background: rgba(74, 124, 89, 0.12); color: #1F3827; font-weight: 800; padding: 3px 8px; border-radius: 12px; font-size: 0.7rem; text-transform: uppercase; display: inline-block; max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;' : 'background: #F1F5F9; color: #475569; font-weight: 700; padding: 3px 8px; border-radius: 12px; font-size: 0.7rem; display: inline-block; max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;'}">${badgeText}</span>
                                </div>
                            </div>
                            ...
                            <div class="flash-card-footer" style="display: flex; flex-direction: column; gap: 6px; margin-top: 6px; padding-top: 6px; border-top: 1px solid rgba(0,0,0,0.06); width: 100%; box-sizing: border-box;">
                                <div style="display: flex; align-items: center; justify-content: space-between; width: 100%; box-sizing: border-box;">
                                    <div class="flash-actions-bar" style="display: flex; align-items: center; gap: 12px;">
                                        <button class="flash-action-btn btn-like-flash ${userHasLiked ? 'liked' : ''}" data-target-id="${targetId}" data-target-type="${targetType}" style="background: none; border: none; color: ${userHasLiked ? '#E76F51' : '#64748B'}; font-weight: 700; font-size: 0.82rem; cursor: pointer; padding: 2px 0; display: inline-flex; align-items: center; gap: 4px;">
                                        ...
                                        <button class="flash-action-btn btn-comments-toggle" data-target-id="${targetId}" data-target-type="${targetType}" style="background: none; border: none; color: #64748B; font-weight: 700; font-size: 0.82rem; cursor: pointer; padding: 2px 0; display: inline-flex; align-items: center; gap: 4px;">
                                        ...
                                        <button class="flash-action-btn btn-share-post" data-target-id="${targetId}" data-title="${(post.title || post.content || '').replace(/"/g, '&quot;')}" style="background: none; border: none; color: #64748B; font-weight: 700; font-size: 0.82rem; cursor: pointer; padding: 2px 0; display: inline-flex; align-items: center;">
                                            <i class="ph ph-share-network" style="font-size: 1.05rem;"></i>
                                        </button>
                                    </div>
                                    ${viewLinkHTML}
                                </div>
                                ${ctaButtonHTML ? `<div style="width: 100%; box-sizing: border-box;">${ctaButtonHTML}</div>` : ''}
                            </div>
