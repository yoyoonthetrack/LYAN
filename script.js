...
safeDomReady(() => {
        ...
        try {
            ...
            feedContainer.innerHTML = filtered.map((post, postIdx) => {
                try {
                    ...
                    return `
                        <div class="flash-card ${isLyann ? 'lyann-card' : ''}" id="${post.id || ''}" style="${isLyann ? 'background: #FAF6F0; border-radius: 20px; padding: 12px 14px; margin-bottom: 12px; border: 1px solid rgba(74, 124, 89, 0.28); box-shadow: 0 4px 16px rgba(15, 23, 18, 0.06), 0 1px 3px rgba(0, 0, 0, 0.03); width: 100%; max-width: 100%; box-sizing: border-box; overflow: hidden;' : 'background: #FFFFFF; border-radius: 20px; padding: 12px 14px; margin-bottom: 12px; border: 1px solid rgba(0, 0, 0, 0.12); box-shadow: 0 4px 16px rgba(0, 0, 0, 0.04), 0 1px 3px rgba(0, 0, 0, 0.02); width: 100%; max-width: 100%; box-sizing: border-box; overflow: hidden;'}">
    ...
    window.openAccountModalSubView = async function(subViewName = 'account') {
        ...
        if (subViewName === 'activity') {
            ...
            `;
            setTimeout(() => {
                if (typeof window.loadAccountFavoritesSubView === 'function') {
                    window.loadAccountFavoritesSubView('ALL');
                }
            }, 50);
    ...
    // === FAVORITES UI CONTROLLER & DELEGATOR ===
    window.loadAccountFavoritesSubView = async function(filterType = 'ALL') {
        const container = document.getElementById('favSubViewContainer');
        const badge = document.getElementById('favCountBadge');
        if (!container) return;

        container.innerHTML = '<div style="grid-column: 1 / -1; text-align: center; padding: 24px; color: #64748B;"><i class="ph ph-circle-notch spinner" style="font-size: 1.5rem;"></i> Chargement de vos favoris...</div>';

        let items = [];
        if (window.LyannFavoritesService && typeof window.LyannFavoritesService.getHydratedFavorites === 'function') {
        ...
        container.innerHTML = filtered.map(item => {
            ...
            return `
                <div class="account-v3-card fav-item-card" data-fav-entity-type="${item.entity_type}" data-fav-entity-id="${item.entity_id}" style="background:#FFF; border:1px solid #E2E8F0; border-radius:16px; padding:14px 16px; display:flex; align-items:center; gap:12px; position:relative; box-shadow: 0 2px 8px rgba(0,0,0,0.03);">
                    ...
                    <button type="button" class="lyann-favorite-btn btn-fav-toggle is-favorite" data-favorite-type="${item.entity_type}" data-favorite-id="${item.entity_id}" data-fav-type="${item.entity_type}" data-fav-id="${item.entity_id}" data-surface="favorites-view" aria-label="Retirer des favoris" title="Retirer des favoris" style="background:none; border:none; color:#4A7C59; font-size:1.3rem; cursor:pointer; padding:10px; min-width:44px; min-height:44px; display:inline-flex; align-items:center; justify-content:center; position:relative; z-index:20; pointer-events:auto; touch-action:manipulation; -webkit-tap-highlight-color:transparent; flex-shrink:0;">
    ...
    window.syncFavoriteButtonStates = async function() {
        if (!window.LyannFavoritesService || typeof window.LyannFavoritesService.getMyFavorites !== 'function') return;
        const favs = await window.LyannFavoritesService.getMyFavorites();
        const favMap = {};
        if (Array.isArray(favs)) {
            favs.forEach(f => {
                if (f && f.entity_type && f.entity_id) {
                    favMap[`${f.entity_type}:${f.entity_id}`] = true;
                }
            });
        }

        document.querySelectorAll('.lyann-favorite-btn, .btn-fav-toggle').forEach(btn => {
            const type = btn.dataset.favoriteType || btn.dataset.favType;
            const id = btn.dataset.favoriteId || btn.dataset.favId;
            if (!type || !id) return;
    ...
    if (!window.__favToggleDelegatorBound__) {
        ...
        let lastFavoriteTapTime = 0;

        async function handleFavoriteBtnTap(e) {
            const favBtn = e.target.closest('.lyann-favorite-btn, .btn-fav-toggle');
            if (!favBtn) return;

            // De-duplicate fast touchend -> click events within 350ms
            const now = Date.now();
            if (now - lastFavoriteTapTime < 350) {
                e.preventDefault();
                e.stopPropagation();
                return;
            }
            lastFavoriteTapTime = now;

            e.preventDefault();
            e.stopPropagation();

            const surface = favBtn.dataset.surface || (favBtn.closest('.flash-card') ? 'bokantaj-feed' : favBtn.closest('.talent-card') ? 'search-explorer' : favBtn.closest('#publicMemberProfileModal') ? 'profile-modal' : favBtn.closest('#lyannDetailModal') ? 'lyann-detail' : 'general');
        ...
        // Bind delegator with capture: true on both touchend and click for immediate iOS WebKit response
        document.addEventListener('touchend', handleFavoriteBtnTap, { passive: false, capture: true });
        document.addEventListener('click', handleFavoriteBtnTap, { capture: true });

        // Tab filter delegator for Account Center Favorites SubView
        document.addEventListener('click', async function(e) {
            const filterBtn = e.target.closest('.fav-tab-btn');
            if (filterBtn) {
                e.preventDefault();
                ...
                filterBtn.style.background = '#1E293B';
                filterBtn.style.color = '#FFF';
                const filter = filterBtn.dataset.favFilter || 'ALL';
                window.loadAccountFavoritesSubView(filter);
            }
        }, { capture: true });
    }
    ...
    // ABSOLUTE AUTH CONTRACT: Supabase Auth Session is the ONLY source of truth.
    async function getActiveSupabaseSession() {
        ...
        try {
            ...
            const isValid = !!(session && session.user && !error);
            console.log('[AUTH_TRACE] getSession result:', {
                hasSession: isValid,
    ...
    window.getActiveSupabaseSession = getActiveSupabaseSession;

    // PROTECTED ROUTE GATE: Require valid Supabase session before allowing access
    async function requireAuthSession(actionName = 'protected action') {
    ...
    // Synchronize Hamburger Drawer & Header elements based strictly on verified session state
    function syncDrawerAuthState(isLoggedIn, profileData) {
        ...
        if (isLoggedIn && profileData) {
            ...
            // Render drawer badge ONLY IF real backend data verifies it
            if (drawerUserBadge) {
                if (profileData.is_pro || profileData.is_pro_verified || profileData.pro_verification_status === 'verified') {
                    drawerUserBadge.innerHTML = '<span class="account-v3-badge account-v3-badge-pro"><i class="ph-fill ph-seal-check"></i> PRO</span>';
                    drawerUserBadge.style.display = 'inline-flex';
                } else if (profileData.is_verified || profileData.verification_status === 'verified') {
                    drawerUserBadge.innerHTML = '<span class="account-v3-badge account-v3-badge-verified"><i class="ph-fill ph-seal-check"></i> Profil vérifié</span>';
                    drawerUserBadge.style.display = 'inline-flex';
                } else {
                    drawerUserBadge.textContent = '';
                    drawerUserBadge.style.display = 'none';
                }
            }

            if (drawerUserAvatar) {
                drawerUserAvatar.src = window.resolveLyannAvatarSrc(profileData.avatar_url);
            }
        } else {
            if (drawerUserName) drawerUserName.textContent = 'Connexion / Inscription';
            if (drawerUserBadge) {
                drawerUserBadge.textContent = '';
                drawerUserBadge.style.display = 'none';
            }
            if (drawerUserAvatar) {
                drawerUserAvatar.src = "data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%20100%20100%22%20width%3D%22100%22%20height%3D%22100%22%3E%3Ccircle%20cx%3D%2250%22%20cy%3D%2250%22%20r%3D%2250%22%20fill%3D%22%23FAF7F2%22%2F%3E%3Ccircle%20cx%3D%2250%22%20cy%3D%2250%22%20r%3D%2248%22%20fill%3D%22%23EBF2ED%22%20stroke%3D%22rgba(74%2C124%2C89%2C0.25)%22%20stroke-width%3D%222%22%2F%3E%3Ccircle%20cx%3D%2250%22%20cy%3D%2238%22%20r%3D%2216%22%20fill%3D%22%234A7C59%22%2F%3E%3Cpath%20d%3D%22M%2022%2084%20C%2022%2066%2C%2034%2058%2C%2050%2058%20C%2066%2058%2C%2078%2066%2C%2078%2084%20Z%22%20fill%3D%22%234A7C59%22%2F%3E%3C%2Fsvg%3E";
            }
        }
    }
    window.syncDrawerAuthState = syncDrawerAuthState;
    ...
    async function updateHeaderAuthState() {
        ...
        if (isLoggedIn && userId) {
            ...
            if (typeof safeStorage !== 'undefined') {
                safeStorage.setItem('lyan_user_logged_in', 'true');
                safeStorage.setItem('lyan_user_id', userId);
            }
            localStorage.setItem('lyan_user_logged_in', 'true');
            localStorage.setItem('lyan_user_id', userId);

            const profileData = await loadAndApplyUserProfile(userId, session);
            syncDrawerAuthState(true, profileData);
...
window.openLyannDetailModal = async function(requestId, initialData = null) {
        ...
        modal.innerHTML = `
            <div class="modal-card" style="max-width: 540px; width: 90%; background: #FFF; border-radius: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.15); overflow: hidden; position: relative;">
                <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; border-bottom: 1px solid #E2E8F0; background: #FAFDFB;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <button type="button" class="lyann-favorite-btn btn-fav-toggle" id="lyannDetailFavBtn" data-favorite-type="REQUEST" data-favorite-id="${requestId}" data-fav-type="REQUEST" data-fav-id="${requestId}" data-surface="lyann-detail" aria-label="Ajouter aux favoris" title="Ajouter aux favoris" style="background: none; border: none; padding: 6px; font-size: 1.3rem; cursor: pointer; color: #94A3B8; display: inline-flex; align-items: center; justify-content: center; width: 44px; height: 44px; min-width: 44px; min-height: 44px; position: relative; z-index: 20; pointer-events: auto; touch-action: manipulation; -webkit-tap-highlight-color: transparent; flex-shrink: 0;">
                            <i class="ph ph-bookmark-simple"></i>
                        </button>
    ...
    if (favDetailBtn && requestData.id) {
        favDetailBtn.dataset.favoriteId = requestData.id;
        favDetailBtn.dataset.favId = requestData.id;
        favDetailBtn.dataset.favoriteType = 'REQUEST';
        favDetailBtn.dataset.favType = 'REQUEST';
    }
    if (typeof window.syncFavoriteButtonStates === 'function') {
        window.syncFavoriteButtonStates();
    }
