-- ============================================================================
-- LYANN — MIGRATION 07 V2.6.2 : PAYMENT CORE (CANONICAL SINGLE PAYMENT_STATUS)
-- ============================================================================
-- IMPORTANT : Ce script adapte la table legacy `public.payments` (actuellement vide)
-- et aligne `public.milestones` vers la machine d'état V2.6.2 finale.
-- CANONICAL SINGLE STATUS : Suppression définitive de `status` au profit de `payment_status`.
-- STRATÉGIE FK : ADD COLUMN sans REFERENCES inline + ADD CONSTRAINT explicite.
-- EXPICIT SET NOT NULL : Forçage systématique des contraintes NOT NULL sur les colonnes.
...
-- 1. ALIGNEMENT DES CONTRAINTES DE PUBLIC.MILESTONES (STATUTS MÉTIER V2.6.2)
...
-- 3. RESTRUCTURATION DES COLONNES DE PUBLIC.PAYMENTS (SINGLE CANONICAL PAYMENT_STATUS)
-- Suppression définitive de la colonne 'status' (legacy) pour éliminer tout doublon
...
ALTER TABLE public.payments
  ...
  ADD COLUMN IF NOT EXISTS service_amount_cents bigint,
  ADD COLUMN IF NOT EXISTS customer_fee_cents bigint,
  ADD COLUMN IF NOT EXISTS customer_total_cents bigint,
  ADD COLUMN IF NOT EXISTS provider_fee_cents bigint,
  ADD COLUMN IF NOT EXISTS provider_net_cents bigint,
  ADD COLUMN IF NOT EXISTS lyann_revenue_cents bigint,
  ADD COLUMN IF NOT EXISTS amount_refunded_cents bigint DEFAULT 0,
  ADD COLUMN IF NOT EXISTS currency text DEFAULT 'EUR',
  ADD COLUMN IF NOT EXISTS payment_status text DEFAULT 'CREATED',
  ADD COLUMN IF NOT EXISTS transfer_status text DEFAULT 'NOT_STARTED',
...
-- FORÇAGE SYSTEMATIQUE ET EXPLICITE DES CONTRAINTES NOT NULL
-- (Garantit que même si une colonne existait déjà comme Nullable post-V2, elle devient NOT NULL)
ALTER TABLE public.payments
  ...
  ALTER COLUMN service_amount_cents SET NOT NULL,
  ALTER COLUMN customer_fee_cents SET NOT NULL,
  ALTER COLUMN customer_total_cents SET NOT NULL,
  ALTER COLUMN provider_fee_cents SET NOT NULL,
  ALTER COLUMN provider_net_cents SET NOT NULL,
  ALTER COLUMN lyann_revenue_cents SET NOT NULL,
  ALTER COLUMN amount_refunded_cents SET NOT NULL,
  ALTER COLUMN currency SET NOT NULL,
  ALTER COLUMN payment_status SET NOT NULL,
  ALTER COLUMN transfer_status SET NOT NULL;
...
-- 5. CONTRAINTES DE SÉCURITÉ ET COMPTABILITÉ FINANCIÈRE V2.6.2 (IDEMPOTENTES)
ALTER TABLE public.payments
...
-- 6. GESTION DES PAYMENT ATTEMPTS & UNICITÉ PAR JALON / PARTIE (INDEX UNIQUE PARTIEL V2.6.2)
-- Exclut FAILED, CANCELLED et REFUNDED pour autoriser un nouveau paiement si nécessaire.
CREATE UNIQUE INDEX IF NOT EXISTS idx_payments_one_active_or_succeeded_per_milestone 
ON public.payments (milestone_id) 
WHERE payment_status IN ('CREATED', 'REQUIRES_ACTION', 'PROCESSING', 'SUCCEEDED', 'PARTIALLY_REFUNDED', 'DISPUTED');
